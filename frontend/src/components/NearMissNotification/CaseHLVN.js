    ...
    const renderTextWithHighlights = (text, currentLength) => {
        ...
        if (highlightTexts) {
            ...
            for (const highlight of highlightTexts) {
                ...
                if (highlightIndex !== -1) {
                    ...
                    if (highlightIndex > tempIndex) {
                        ...
                        if (normalLength > 0) {
                            result += `<span style="color: #ffffff; font-size: 2.4rem; line-height: 1.3;">${normalText.slice(0, normalLength)}</span>`;
                        }
            ...
            if (tempIndex < currentLength) {
                ...
                result += `<span style="color: #ffffff; font-size: 2.4rem; line-height: 1.3;">${remainingText}</span>`;
            }
            ...
            if (highlightIndex !== -1) {
                ...
                // 강조 텍스트가 없는 경우
                result = `<span style="color: #ffffff; font-size: 2.4rem; line-height: 1.3;">${text.slice(0, currentLength)}</span>`;
            }
        } else {
            // 강조 텍스트가 없는 경우
            result = `<span style="color: #ffffff; font-size: 2.4rem; line-height: 1.3;">${text.slice(0, currentLength)}</span>`;
...
const CaseHLVN = () => {
    ...
    const messages = [
        ...
        "At the time, our village had a 16-foot levee in place. By sheer luck, the storm shifted course and struck another village located 5 miles away. As a result, the river near us rose to 15 feet, and our village remained undamaged, experiencing 0% flood loss.",
        "Unfortunately, the other village suffered devastating floods and severe economic losses. Most of their wealth was lost during the event."
    ];
    ...
    const calculateLinePosition = (index) => {
        ...
        console.log(`Line ${index} position calculation:`, totalHeight, 'lineHeights:', lineHeights);
        return totalHeight;
    };

    const handleLineHeightUpdate = (index, height) => {
        console.log(`Line ${index} height update:`, height);
        console.log('Current lineHeights:', lineHeights);
        setLineHeights(prev => ({
            ...prev,
            [index]: height
        }));
                    ...
                    ref={(el) => {
                        ...
                        {index < currentLineIndex ? (
                            ...
                            <>
                                ...
                                {index === 2 && (
                                    <>
                                        <span style={normalText}>At the time, our village had a </span><span style={highlightNumber}>16-foot</span><span style={normalText}> levee in place. By sheer luck, the storm shifted course and struck another village located 5 miles away. As a result, the river near us rose to </span><span style={highlightNumber}>15 feet</span><span style={normalText}>, and </span><span style={highlightResultWithUnderline}>our village</span><span style={highlightResult}> remained undamaged</span><span style={normalText}>, experiencing </span><span style={highlightResult}>0% flood loss</span><span style={normalText}>.</span>
                                    </>
                                )}
                                {index === 3 && (
                                    <>
                                        <span style={normalText}>Unfortunately, the </span><span style={highlightResultWithUnderline}>other village</span><span style={highlightResult}> suffered devastating floods and severe economic losses</span><span style={normalText}>. Most of their wealth was lost during the event.</span>
                                    </>
                                )}
                            </>
                        ) : index === currentLineIndex ? (
                            ...
                            >
                                ...
                                {index === 2 && (
                                    <TypingText 
                                        text="At the time, our village had a 8-foot levee in place. By sheer luck, the storm shifted course and struck another village located 5 miles away. As a result, the river near us rose only to 7 feet, and our village remained undamaged, experiencing 0% flood loss." 
                                        onComplete={handleLineComplete}
                                        highlightTexts={[
                                            { text: "8-foot", style: highlightNumber },
                                            { text: "7 feet", style: highlightNumber },
                                            { text: "our village remained undamaged", style: highlightResultWithUnderline },
                                            { text: "0% flood loss", style: highlightResult }
                                        ]}
                                    />
                                )}
                                {index === 3 && (
                                    <TypingText 
                                        text="Unfortunately, the other village suffered devastating floods and severe economic losses. Most of their wealth was lost during the event." 
                                        onComplete={handleLineComplete}
                                        highlightTexts={[
                                            { text: "other village suffered devastating floods and severe economic losses", style: highlightResultWithUnderline }
                                        ]}
                                    />
                                )}
                            </div>
